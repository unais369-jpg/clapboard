...
// Clapboard v5: PDF + DOCX + TXT screenplay reader
(function(){
 const input=document.getElementById("scriptFile"), status=document.getElementById("fileStatus"), btn=document.getElementById("analyzeScript");
 if(!input||!status||!btn)return;
 ...
 async function readPDF(file){
   ...
   for(let i=1;i<=pdf.numPages;i++){
     ...
     text.push(content.items.map(x=>x.str).join(" "));
   }
   return normalizeText(text.join("\n"));
 }
 // Use a fresh handler. It accepts PDF and enables Analyze as soon as a file is selected.
 input.addEventListener("change", async function(){
   const f=this.files && this.files[0];
   if(!f){status.textContent="No screenplay uploaded";btn.disabled=true;return;}
   btn.disabled=false; status.textContent=f.name+" • selected";
   try{
     if(/\.pdf$/i.test(f.name)){
       status.textContent=f.name+" • reading PDF…";
       screenplayText=await readPDF(f);
     }else if(/\.docx$/i.test(f.name)){
       if(!window.mammoth){
         await new Promise((resolve,reject)=>{const s=document.createElement("script");s.src="https://cdn.jsdelivr.net/npm/mammoth@1.8.0/mammoth.browser.min.js";s.onload=resolve;s.onerror=reject;document.head.appendChild(s);});
       }
       screenplayText=normalizeText((await mammoth.extractRawText({arrayBuffer:await f.arrayBuffer()})).value);
     }else if(/\.txt$/i.test(f.name)){
       screenplayText=normalizeText(await f.text());

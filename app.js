// Clapboard v15 CLEAN — PDF + DOCX + TXT screenplay analyzer
...
function renderScenes(){
 ...
 tr.innerHTML=fields.map(k=>`<td><input data-i="${i}" data-k="${k}" value="${String(s[k]??"").replaceAll('&',"&amp;").replaceAll('"',"&quot;").replaceAll('<',"&lt;").replaceAll('>',"&gt;")}"></td>`).join("")+`<td><button data-del="${i}">×</button></td>`;
 tb.appendChild(tr)});
...
async function loadExternalScript(urls, testFn, label){
  if(testFn()) return;
  let lastErr=null;
  for(const url of urls){
    try{
      await new Promise((res,rej)=>{
        ...
        s.onload=()=>finish(res); s.onerror=()=>finish(rej,new Error("Failed to load "+url));
        const timer=setTimeout(()=>finish(rej,new Error("Timeout loading "+url)),12000);
        document.head.appendChild(s);
      });
      if(testFn()) return;
    }catch(e){lastErr=e;}
  }
  throw new Error(label+" library could not be loaded. Please check your internet connection and try again.");
}
async function readFile(file){
  ...
  if(/\.pdf$/i.test(name)){
    ...
    // Run without a worker so iPhone/Safari does not fail on cross-origin worker loading.
    const pdf=await window.pdfjsLib.getDocument({data,disableWorker:true,useWorkerFetch:false,isEvalSupported:true}).promise;
    const pages=[];
    for(let i=1;i<=pdf.numPages;i++){
      ...
      pages.push(rows.map(r=>r.parts.sort((a,b)=>a.x-b.x).map(x=>x.str).join(" ")).join("\n"));
    }
    const text=normText(pages.join("\n\n"));
    if(!text) throw new Error("The PDF contains no selectable text. It may be a scanned/image-only PDF.");
    return text;
  }
  if(/\.doc$/i.test(name)) throw new Error("Legacy .doc is not supported in Safari. Please save it as .docx or PDF.");
...
(function(){
  ...
  input.addEventListener("change",async e=>{
    ...
    try{
      ...
      console.error(err);
      screenplayText="";
      status.textContent=f.name+" • ERROR: "+(err&&err.message?err.message:"Could not read file");
      btn.disabled=true;
    }
  });
  btn.onclick=()=>{ if(!screenplayText || btn.disabled) return; btn.disabled=true; btn.textContent="Analyzing…"; status.textContent=(input.files[0]?.name||"Screenplay")+" • analyzing…"; try{ analyzeScreenplay(); status.textContent=(input.files[0]?.name||"Screenplay")+" • analysis complete"; }catch(err){ console.error(err); status.textContent=(input.files[0]?.name||"Screenplay")+" • ERROR: "+(err?.message||"Analysis failed"); alert("Analysis failed: "+(err?.message||"Unknown error")); } finally { btn.disabled=false; btn.textContent="✓ Analysis Complete"; }};
})();
load();

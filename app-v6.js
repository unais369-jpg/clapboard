...
function splitScenes(t){
  const ls=norm(t).split("\n").map(x=>x.trim()).filter(Boolean);
  const re=/^(INT\.?|EXT\.?|INT\/EXT\.?|I\/E\.?)\b/i;
  let out=[],c=null;
  for(const l of ls){
    if(re.test(l)){ if(c)out.push(c); c={h:l,ls:[]}; }
    else if(c)c.ls.push(l);
  }
  if(c)out.push(c);
  return out;
}
function head(h){
  ...
  const ie=/^INT\/EXT|^I\/E/.test(x)?"INT/EXT":/^INT/.test(x)?"INT":/^EXT/.test(x)?"EXT":"";
  const tm=/\b(NIGHT|NITE)\b/.test(x)?"NIGHT":/\b(MORNING|DAWN)\b/.test(x)?"MORNING":/\b(EVENING|DUSK)\b/.test(x)?"EVENING":/\b(AFTERNOON)\b/.test(x)?"AFTERNOON":/\b(DAY|DAYTIME)\b/.test(x)?"DAY":"";
  const loc=x.replace(/^(INT\/EXT|I\/E|INT|EXT)\.?\s*/,"").replace(/\s*[-–—]\s*(DAY|NIGHT|NITE|MORNING|EVENING|AFTERNOON|DAWN|DUSK)\b.*$/,'').trim();
  return {ie,time:tm,loc:loc||"UNSPECIFIED"};
}
function chars(ls){
  const o=[];
  for(const l of ls){
    const m=l.match(/^([A-Z][A-Z0-9 .,'&-]{1,35})\s*(?:\([^)]*\))?$/);
    if(m&&!/^(CUT TO|FADE|DISSOLVE|CONTINUED|INT|EXT|DAY|NIGHT|MORNING|EVENING|SCENE|THE END)$/.test(m[1].trim())&&!o.includes(m[1].trim()))o.push(m[1].trim());
  }
  return o.slice(0,12);
}
function words(t,w){
  const f=w.filter(x=>new RegExp("\\b"+x.replace("-","\\-")+"\\b","i").test(t));
  return f.map(x=>x[0].toUpperCase()+x.slice(1)).join(", ")||"—";
}
function analyzeScreenplay(){
  if(!screenplayText){ alert("Please choose a screenplay first."); return; }
  const bs=splitScenes(screenplayText);
  if(!bs.length){ alert("No standard INT/EXT scene headings were detected."); return; }
  scenes=bs.map((b,i)=>{
    const p=head(b.h), body=b.ls.join(" ");
    return {
      no:i+1,loc:p.loc,ie:p.ie,time:p.time||"UNSPECIFIED",
      chars:chars(b.ls).join(", ")||"Review screenplay",artists:"To be cast",
      props:words(body,["phone","mobile","gun","pistol","bag","file","files","laptop","key","watch","wallet","bottle","glass","cup","book","letter","photo","umbrella","helmet","bike"]),
      costume:words(body,["black","white","blue","red","green","yellow","shirt","t-shirt","pants","jeans","dress","saree","uniform","jacket"]),
      vehicle:words(body,["car","bike","motorcycle","scooter","auto","taxi","bus","jeep","truck","van","innova"]),
      camera:words(body,["gimbal","drone","steadicam","handheld","35mm","50mm","85mm","macro","crane"])||"Standard camera setup"
    };
  });
  renderScenes(); generateSchedule(); save(); renderStats();
  alert(`Clapboard analyzed ${scenes.length} scene(s). Please review the breakdown.`);
}

async function loadScriptText(file){
  const name=file.name||"";
  if(/\.txt$/i.test(name)) return norm(await file.text());
  if(/\.docx$/i.test(name)){
    if(!window.mammoth){
...
(function(){
  ...
  input.onchange=async e=>{
    ...
    try{
      ...
      screenplayText=await loadScriptText(f);
